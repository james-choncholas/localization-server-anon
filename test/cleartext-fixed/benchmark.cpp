...
bool withinRel(float v, float t) {
  return fabs(v-t) <= std::max(localization_tol_rel, fabs(localization_tol_rel * std::max(v,t)));
}

template<typename T>
bool convert_and_localize(vector<cv::Point3_<float>> _objectPoints,
                          vector<cv::Point_<float>> _imagePoints,
                          float _f, float _cx, float _cy, T* _x,
                          auto cleartext_localize_func,
                          vector<float> &gtpose, string logprint="") {
...
int eth3d_localize(NetIO* io, int party,
                   int num_frames, int num_trials, int max_num_pts,
  ...
  //for (int l=0; l<1; l++) {
    ...
    for (int frame=0; frame<test_num_frames; frame++) {
      ...
      for (int num_pts=6; num_pts<=test_num_pts; num_pts+=MAX((test_num_pts-6)/5, 1)) { // at most 5 intervals
        for (int t=0; t<num_trials; t++) {
...

          {
            ...
            bool good = convert_and_localize(objectPoints, imagePoints, f, cx, cy, &initialGuessCopy[0], gaussNewton<double>, gtpose);
            if (!silent) {
...

          {
            ...
            bool good = convert_and_localize(objectPoints, imagePoints, f, cx, cy, &initialGuessCopy[0], lm<double>, gtpose);
            if (!silent) {
...

          {
            ...
            bool good = convert_and_localize(objectPoints, imagePoints, f, cx, cy, &initialGuessCopy[0], gaussNewton<float>, gtpose);
            if (!silent) {
              printVector("result:\n", &initialGuessCopy[0], 6);
            }
            if (good) {
...

          {
            ...
            bool good = convert_and_localize(objectPoints, imagePoints, f, cx, cy, &initialGuessCopy[0], lm<float>, gtpose);
            if (!silent) {
              printVector("cleartext result:\n", &initialGuessCopy[0], 6);
            }
            bool fail = false;
...

          {
            ...
            bool good = convert_and_localize(objectPoints, imagePoints, f, cx, cy, &initialGuessCopy[0], gaussNewton<fixed_point<int64_t,32>>, gtpose);
            if (!silent) {
              printVector("result:\n", &initialGuessCopy[0], 6);
            }
            if (good) {
          ...
          }
          {
            ...
            bool good = convert_and_localize(objectPoints, imagePoints, f, cx, cy, &initialGuessCopy[0], lm<fixed_point<int64_t,32>>, gtpose);
            if (!silent) {
              printVector("result:\n", &initialGuessCopy[0], 6);
            }
            if (good) {
...

          {
            ...
            bool good = convert_and_localize(objectPoints, imagePoints, f, cx, cy, &initialGuessCopy[0], gaussNewton<fixed_point<int32_t,16>>, gtpose);
            if (!silent) {
              printVector("result:\n", &initialGuessCopy[0], 6);
            }
            if (good) {
          ...
          }
          {
            ...
            bool good = convert_and_localize(objectPoints, imagePoints, f, cx, cy, &initialGuessCopy[0], lm<fixed_point<int32_t,16>>, gtpose);
            if (!silent) {
              printVector("result:\n", &initialGuessCopy[0], 6);
            }
            if (good) {
  ...
  if (!silent) {
    MSG("SeNtInAl,xy,%s,%s,%d,%u\n", __FUNCTION__, "cv_successes", 0, cv_successes);
    MSG("SeNtInAl,xy,%s,%s,%d,%u\n", __FUNCTION__, "gn_double_successes", 0, gn_double_successes);
    MSG("SeNtInAl,xy,%s,%s,%d,%u\n", __FUNCTION__, "lm_double_successes", 0, lm_double_successes);
    MSG("SeNtInAl,xy,%s,%s,%d,%u\n", __FUNCTION__, "gn_float_successes", 0, gn_float_successes);
    MSG("SeNtInAl,xy,%s,%s,%d,%u\n", __FUNCTION__, "lm_float_successes", 0, lm_float_successes);
    MSG("SeNtInAl,xy,%s,%s,%d,%u\n", __FUNCTION__, "gn_fixed32_successes", 0, gn_fixed32_successes);
    MSG("SeNtInAl,xy,%s,%s,%d,%u\n", __FUNCTION__, "lm_fixed32_successes", 0, lm_fixed32_successes);
    MSG("SeNtInAl,xy,%s,%s,%d,%u\n", __FUNCTION__, "gn_fixed64_successes", 0, gn_fixed64_successes);
    MSG("SeNtInAl,xy,%s,%s,%d,%u\n", __FUNCTION__, "lm_fixed64_successes", 0, lm_fixed64_successes);

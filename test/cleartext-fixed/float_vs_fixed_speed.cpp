...
#define TEST_SZ 1000*10
    ...
    #define START() \
        ...
        WALL_TIC(EMP_ADD);
    #define STOP(LOGSTR_) \
        WALL_TOC_CSV_GROUPED_BAR(EMP_ADD,LOGSTR_);
#endif
    ...
    #define START() \
        ...
        WALL_TIC(EMP_MUL);
    #define STOP(LOGSTR_) \
        WALL_TOC_CSV_GROUPED_BAR(EMP_MUL,LOGSTR_);
#endif
...
int main(int argc, char** argv) {
...

    {
        ...
        Float *s_testa = static_cast<Float*>(operator new[](TEST_SZ * sizeof(Float)));
        Float *s_testb = static_cast<Float*>(operator new[](TEST_SZ * sizeof(Float)));
        Float *s_testc = static_cast<Float*>(operator new[](TEST_SZ * sizeof(Float)));

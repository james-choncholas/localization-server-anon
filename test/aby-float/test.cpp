...
const vector<e_sharing> ctypes = {/*S_ARITH,*/ S_BOOL, S_YAO}; // no floats with arith
...
void aby_matmult(e_role role, e_sharing sharing, cv::Mat A, cv::Mat B) {
  ...
  matmult(A.ptr<float>(),A.rows,A.cols,
          B.ptr<float>(),B.rows,B.cols,
          C.ptr<float>());
  ...
  for (int i=0; i<m*nn; i++) {
    ...
    REQUIRE(out_bitlen == 32);
    REQUIRE(out_nvals == 1);
    REQUIRE_THAT(*(float*)output, WithinAbs(C.ptr<float>()[i], float_test_epsilon));
  }
  delete party;
}
...
void aby_rod(e_role role, e_sharing sharing, float* r) {
  ...
  for (int i=0; i<12; i++) {
    ...
    REQUIRE(out_bitlen == 32);
    REQUIRE(out_nvals == 1);
    REQUIRE_THAT(*(float*)output, WithinAbs(clear_res.ptr<float>()[i], float_test_epsilon));
  }
...
void aby_proj(e_role role, e_sharing sharing, cv::Mat cameraMatrix, cv::Mat distCoeffs,
              vector<cv::Point2f> imagePoints, vector<cv::Point3f> objectPoints) {
  ABYParty* party = new ABYParty(role, address, port, slvl, bitlen, nthreads, mt_alg, reservegates, get_circuit_dir());
  std::vector<Sharing*>& sharings = party->GetSharings();
  Circuit* circ = sharings[sharing]->GetCircuitBuildRoutine();
  ...
  P( cv::Range(0,3), cv::Range(0,numPoints) ) = matOP.t(); // copy points into new matrix
  cv::Mat projected = cv::Mat(3, numPoints, cv::DataType<float>::type);
  projectPoints(P.ptr<float>(), x.ptr<float>(), cameraMatrix.ptr<float>(), projected.ptr<float>(),numPoints);
  for (int i=0; i<numPoints; ++i) {
    REQUIRE_THAT(imagePoints[i].x, WithinAbs(projected.at<float>(0,i), reprojection_tol));
    REQUIRE_THAT(imagePoints[i].y, WithinAbs(projected.at<float>(1,i), reprojection_tol));
  }
  ...
  for (int i=0; i<numPoints; ++i) {
    ...
    REQUIRE(out_bitlen == 32);
    REQUIRE(out_nvals == 1);
    REQUIRE_THAT(*(float*)output, WithinAbs(imagePoints[i].x, reprojection_tol));

    assert(res[i+numPoints] != NULL);
    res[i+numPoints]->get_clear_value_vec(&output, &out_bitlen, &out_nvals);
    REQUIRE(out_bitlen == 32);
    REQUIRE(out_nvals == 1);
    REQUIRE_THAT(*(float*)output, WithinAbs(imagePoints[i].y, reprojection_tol));
  }
...
TEST_CASE("ABY point projection is computed", "[aby_proj]") {
  ...
  std::thread bob([&]() {
    for (auto ctype : ctypes) {
      aby_proj(CLIENT, ctype, cameraMatrix, distCoeffs, imagePoints, objectPoints);
    }
  });

  for (auto ctype : ctypes) {
    aby_proj(SERVER, ctype, cameraMatrix, distCoeffs, imagePoints, objectPoints);
  }
  bob.join();
}
...
void aby_invert(e_role role, e_sharing sharing, float *in, int m, int n) {
  ...
  for(int i=0; i<n; i++) {
    for (int j=0; j<m; j++) {
      REQUIRE_THAT(cvres.at<float>(i,j), WithinAbs(res[i][j], float_test_epsilon));
    }
  }
  ...
  BuildInvertCircuit(s_in, m, n, s_res,
...
void aby_localize(e_role role, e_sharing sharing,
  ...
  if (role == SERVER) {
    ...
    for(int p=0; p<numPts; p++) {
      s_objectPoints[p] = bc->PutINGate((uint32_t*) &objectPoints[p].x, bitlen, role);
    }
    for(int p=0; p<numPts; p++) {
      s_objectPoints[numPts+p] = bc->PutINGate((uint32_t*) &objectPoints[p].y, bitlen, role);
    }
    for(int p=0; p<numPts; p++) {
      s_objectPoints[(2*numPts)+p] = bc->PutINGate((uint32_t*) &objectPoints[p].z, bitlen, role);
    }
    //for(int p=0; p<numPts; p++) {
    //    s_objectPoints[3*numPts+p] = bc->PutINGate((uint32_t*) &one, bitlen, role);
    //}
    for(int p=0; p<numPts; p++) {
      s_imagePoints[p] = bc->PutINGate((uint32_t*) &imagePoints[p].x, bitlen, role);
    }
    for(int p=0; p<numPts; p++) {
      s_imagePoints[numPts+p] = bc->PutINGate((uint32_t*) &imagePoints[p].y, bitlen, role);
    }
    //for(int p=0; p<numPts; p++) {
    //    s_imagePoints[2*numPts+p] = bc->PutINGate((uint32_t*) &one, bitlen, role);
    //}
  ...
  secure_localize_func(s_objectPoints, s_imagePoints, numPts,
                       s_f, s_cx, s_cy, s_x,
                       (BooleanCircuit*) circ, party, role);
  ...
  std::thread bob([&]() {
    for (auto ctype : ctypes) {
      aby_localize(CLIENT, ctype, rvec, tvec, cameraMatrix, distCoeffs,
                   objectPoints, imagePoints, gaussNewton<float>, BuildAndRunGaussNewton);
    }
  });

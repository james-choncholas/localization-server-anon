...
TEST_CASE("EMP Gauss Newton pose estimation is computed", "[emp_gn]") {
  ...
  std::thread bob([&]() {
    NetIO *io = new NetIO("127.0.0.1", port);
    emp_localize(io, BOB, rvec, tvec, cameraMatrix, distCoeffs, objectPoints, imagePoints, gaussNewton<float>, BuildGaussNewton);
    delete io;
  });

  NetIO *io = new NetIO(nullptr, port);
  emp_localize(io, ALICE, rvec, tvec, cameraMatrix, distCoeffs, objectPoints, imagePoints, gaussNewton<float>, BuildGaussNewton);
  bob.join();
  delete io;
}

...
void emp_trig(NetIO* io, int party, float angle) {
  ...
  REQUIRE_THAT(sinres.reveal<double>(), WithinAbs(sin(angle), float_test_epsilon));

  Float cosres = BuildCosCircuit(v);
  REQUIRE_THAT(cosres.reveal<double>(), WithinAbs(cos(angle), float_test_epsilon));
...
void emp_mat(NetIO* io, int party, cv::Mat A, cv::Mat B) {
  ...
  BuildMatmultCircuit(eA,A.rows,A.cols,
                      eB,B.rows,B.cols,
                      eres);

  for (int i=0; i<A.rows*B.cols; ++i) {
    REQUIRE_THAT(eres[i].reveal<double>(), WithinAbs(clear_res.ptr<float>()[i], float_test_epsilon));
  }
...
void emp_rod(NetIO* io, int party, float* r) {
  ...
  for (int i=0; i<12; ++i) {
    ...
    REQUIRE_THAT(sR[i].reveal<double>(), WithinAbs(R.ptr<float>()[i], float_test_epsilon));
  }
  delete[] sr;
  delete[] sR;
...
void emp_proj(NetIO* io, int party,
  ...
  for (int i=0; i<2*nOp; ++i) {
    REQUIRE_THAT(projected.ptr<float>()[i], WithinAbs(sres[i].reveal<double>(), reprojection_tol));
  }
  delete[]sP;
  delete[]sx;
  delete[]sK;
...
void emp_svd_sign(NetIO* io, int party, float a, float b) {
  ...
  REQUIRE_THAT(sa.reveal<double>(), WithinAbs(cleartext_res, float_test_epsilon));
  finalize_semi_honest();
}
...
void emp_svd_pythag(NetIO* io, int party, float a, float b) {
  ...
  REQUIRE_THAT(out.reveal<double>(), WithinAbs(cleartext_res, float_test_epsilon));
  finalize_semi_honest();
}
...
void emp_invert(NetIO* io, int party, float *in, int m, int n) {
  ...
  for(int i=0; i<n; i++) {
    for (int j=0; j<m; j++) {
      REQUIRE_THAT(sres[i][j].reveal<double>(), WithinAbs(res[i][j], float_test_epsilon));
    }
    delete[] res[i];
    delete[] sres[i];
  }
...
void emp_localize(NetIO* io, int party,
  ...
  Float *simagePoints = static_cast<Float*>(operator new[](3*numPts * sizeof(Float)));

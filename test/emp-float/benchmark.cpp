...
int eth3d_localize(NetIO* io, int party,
      ...
      for (int num_pts=6; num_pts<=test_num_pts; num_pts+=MAX((test_num_pts-6)/5, 1)) { // at most 5 intervals
        for (int t=0; t<num_trials; t++) {
...

          {
            ...
            CLOCK(cleartext);
            TIC(cleartext);
            cleartext_localize_func(objectPoints, imagePoints, f, cx, cy, &initialGuessCopy[0], "");
            TOC(cleartext);
            if (!silent) {
...

          {
            ...
            Float *sobjectPoints = static_cast<Float*>(operator new[](4*num_pts * sizeof(Float)));
            Float *simagePoints = static_cast<Float*>(operator new[](3*num_pts * sizeof(Float)));
            for(int i=0; i<num_pts; i++) {
                // [x1, x2... ; y1, y2... ; z1, z2...]
                sobjectPoints[i] = Float(objectPointsSubset[i].x, ALICE);
                sobjectPoints[num_pts + i] = Float(objectPointsSubset[i].y, ALICE);
                sobjectPoints[2*num_pts + i] = Float(objectPointsSubset[i].z, ALICE);
            ...
            auto [converged, num_loc_iterations] = secure_localize_func(sobjectPoints, simagePoints,
                num_pts, sf, scx, scy, sx);
            ...
            if (!silent) {
              if (good) {
                ...
                MSG("SeNtInAl,xy,%s,%s,%d,%g\n", __FUNCTION__, timer_name.c_str(), num_pts,
                    std::chrono::duration_cast<std::chrono::microseconds>(toc - tic).count() / 1000000.0);
                MSG("SeNtInAl,xy,%s,%s,%d,%g\n", __FUNCTION__, (timer_name + "_per_loc_itr").c_str(), num_pts,
                    std::chrono::duration_cast<std::chrono::microseconds>(toc - tic).count() / (1000000.0 * num_loc_iterations));

                ++secure_successes;
                MSG("SeNtInAl,grouped_bar,%s,%s%s,%d,%d\n", __FUNCTION__, log_str.c_str(),"_additions", num_pts, num_additions);
                MSG("SeNtInAl,grouped_bar,%s,%s%s,%d,%d\n", __FUNCTION__, log_str.c_str(),"_subtractions", num_pts, num_subtractions);
                MSG("SeNtInAl,grouped_bar,%s,%s%s,%d,%d\n", __FUNCTION__, log_str.c_str(),"_multiplications", num_pts, num_multiplications);
                MSG("SeNtInAl,grouped_bar,%s,%s%s,%d,%d\n", __FUNCTION__, log_str.c_str(),"_divisions", num_pts, num_divisions);
                MSG("SeNtInAl,xy,%s,%s%s,%d,%lu\n", __FUNCTION__, log_str.c_str(),"_bytes_tx", num_pts, io->size_tx);
                MSG("SeNtInAl,xy,%s,%s%s,%d,%lu\n", __FUNCTION__, log_str.c_str(),"_bytes_rx", num_pts, io->size_rx);
              } else {
  ...
  if (!silent) {
    ...
    MSG("SeNtInAl,xy,%s,%s,%d,%u\n", __FUNCTION__, "cleartext_successes", 0, cleartext_successes);
    MSG("SeNtInAl,xy,%s,%s,%d,%u\n", __FUNCTION__, "secure_successes", 0, secure_successes);
...
int main(int argc, char** argv) {
  ...
  std::thread bob([&]() {
    ...
    eth3d_localize(io, BOB, num_frames, num_trials, max_num_pts,
                   clear_func, secure_func, true, log_str);
    delete io;
  });

  NetIO *io = new NetIO(nullptr, port);
  eth3d_localize(io, ALICE, num_frames, num_trials, max_num_pts,
                 clear_func, secure_func, false, log_str);
  bob.join();
  delete io;
}

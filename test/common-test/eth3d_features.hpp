...
public:
    ETH3DFeatures(const std::string& base_path, const std::string& location) {
        ...
        std::string images_txt_path = base_path + location + calib_path + "images.txt";
        std::string points3d_txt_path = base_path + location + calib_path + "points3D.txt";
        ...
        while (!images_file_stream.eof() && !images_file_stream.bad()) {
            ...
            new_image->image_T_global = DualQuatd::createFromAngleAxisTrans(
                                    iTg_rot.norm(), // angle is the norm of rotation vector or quaternion?
    ...
    void worldFeatures(std::vector<Point3_<T>>& points) {
        ...
        for (auto& obs : images[id]->observations) {
            if (obs.point3d_id == -1) continue; // no correspondance found by dataset owners

...
Float LMIteration(Float threeDPts[], Float y0[], int numPts,
                  Float &f, Float &cx, Float &cy, Float &lambda,
                  Float x[])
{
...
void lm_server(int party, NetIO *io, NetIO *ttpio)
{
    ...
    Float *s_threeDPts = static_cast<Float *>(operator new[](4 * numPts * sizeof(Float) + 100));
    Float *s_twoDPts = static_cast<Float *>(operator new[](3 * numPts * sizeof(Float) + 100));
    ...
    Float s_errNorm = LMIteration(s_threeDPts, s_twoDPts, numPts, s_f, s_cx, s_cy, s_lambda, s_x);
#endif
    double interval = time_from(start);

...
//if (i==2) cout << "a[i][i] " << a[i][i].reveal<double>() << endl; good
                f = a[i][i]; // no .If okay
                Float temp = s.sqrt();
...
//if (i == 2) return 0; // bad here
        ...
        if (i < nRows && i != nCols - 1) {
            ...
            /*if (scale)*/ {
                ...
                g = temp.If(ifscale, g);
                temp = f * g - s;
                h = temp.If(ifscale, Float(1.0, PUBLIC)); // h is used to set rv1 if scale==0, set to 1 for noop
                temp = f - g;
...
//}
            ...
            for (int l_finder=0; l_finder<k; l_finder++) {
                Bit found_l = priv_l.equal(Integer(32, l_finder, PUBLIC));
                rv1[l_finder] = rv1[l_finder].If(found_l & (!converged), Float(0.0, PUBLIC));
            }
...
uint32_t test_sign_circuit(int party, NetIO *io, float a_in, float b_in) {
    ...
    Float a = Float(a_in, ALICE);
    Float b = Float(b_in, ALICE);

    CLOCK(Run);
    TIC(Run);
    BuildSignCircuit(&a, &b);
    TOC(Run);
...
uint32_t test_pythag_circuit(int party, NetIO *io, float a_in, float b_in) {
    ...
    CLOCK(Run);
    TIC(Run);
    Float out = BuildPythagCircuit(&a, &b);
    TOC(Run);

    cout << out.reveal<double>(PUBLIC) << endl;
...
uint32_t test_svd_circuit(int party, NetIO *io,
    ...
    Float* w = static_cast<Float*>(operator new[](nCols * sizeof(Float)));
    for(int i=0; i<nCols; i++) {
        w[i] = Float(0.0, PUBLIC);
    }
    Float** v = new Float*[nCols];
    for(int i=0; i<nCols; i++) {
        v[i] = static_cast<Float*>(operator new[](nCols * sizeof(Float)));
        for(int j=0; j<nCols; j++) {
            v[i][j] = Float(0.0, PUBLIC);
        }
    }
    ...
    CLOCK(Run);
    TIC(Run);
    BuildSvdCircuit(a, nRows, nCols, w, v);
    TOC(Run);
    ...
    for(int i=0; i<nRows; i++) {
        for (int j=0; j<nCols; j++) {
            a_in[i][j] = a[i][j].reveal<double>(PUBLIC);
            a[i][j].~Float();
        }
        delete[] a[i];
    }
    delete[] a;
    for(int i=0; i<nCols; i++) {
        w_in[i] = w[i].reveal<double>(PUBLIC);
        w[i].~Float();
    }
    delete[] w;
    for(int i=0; i<nCols; i++) {
        for (int j=0; j<nCols; j++) {
            v_in[i][j] = v[i][j].reveal<double>(PUBLIC);
            v[i][j].~Float();
        }
        delete[] v[i];
    }

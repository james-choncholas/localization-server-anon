...
//! >>=  ==>  >> (right_shiftable),
//! <<=  ==>  << (left_shiftable).
class fixed_point_emp :
    public Swappable<fixed_point_emp<I,F>>,
    public Comparable<fixed_point_emp<I,F>>
{
    ...
    //!
    //! The fixed_point_emp class needs 2 to the power of P in several locations in
    ...
    //!
    //! This constructor takes a value of type Integer and initializes the internal
    //! representation of fixed_point_emp<I, F> with it.
    fixed_point_emp(
...
public:
    ...
    fixed_point_emp(
        ...
        // Who owns the value
        int party = PUBLIC)
        : value_(I+F, value * power2<F>::value + (value >= 0 ? .5 : -.5), party)
    { }
    ...
    /// Copy constructor.
    fixed_point_emp(fixed_point_emp<I, F> const& rhs)
        : value_(rhs.value_)
    { }

    /// Copy constructor.
    fixed_point_emp(Integer const& rhs)
        : value_(rhs)
    { }
    ...
    //Swappable
    fixed_point_emp<I, F> select(const Bit & sel, const fixed_point_emp<I, F> & rhs) const {
    ...
    Bit operator !() const
    {
        return value_ == Integer(value_.size(), 0, PUBLIC);
    }
    ...
    /// Addition.
    fixed_point_emp<I, F> & operator +=(
        fixed_point_emp<I, F> const& summand)
    {
        ...
        add_full(value_.bits.data(), nullptr, value_.bits.data(), summand_bits.data(), nullptr, size*2);
        value_.resize(size);
    ...
    /// Subtraction.
    fixed_point_emp<I, F> & operator -=(
        fixed_point_emp<I, F> const& diminuend)
    {
        ...
        sub_full(value_.bits.data(), nullptr, value_.bits.data(), diminuend_bits.data(), nullptr, size*2);
    ...
    /// Multiplication.
    fixed_point_emp<I, F> & operator *=(
        fixed_point_emp<I, F> const& factor)
    {
        int size = (I+F);
...
//        //// to take advantage of operators only being size bits (not 2*size).
//        Bit * sum = new Bit[size*2];
...
//
//            sum[size+i] = sum[size+i-1]; // sign extend sum by one position
//
...
//        return *this;
    ...
    friend fixed_point_emp<I, F> cos(
        ...
        fixed_point_emp<I, F> x)
    {
        fixed_point_emp<I, F> x_ =
            fmod(x, fixed_point_emp<I, F>(M_PI * 2));
        ...
        fixed_point_emp<I, F> y = - xx *
            fixed_point_emp<I, F>(1. / (2 * 3 * 4 * 5 * 6));
        ...
        fixed_point_emp<I, F> x)
    {
        ...
        fixed_point_emp<I, F> y = - xx *
            fixed_point_emp<I, F>(1. / (2 * 3 * 4 * 5 * 6 * 7));
    ...
    friend fixed_point_emp<I, F> sqrt(
        fixed_point_emp<I, F> x)
    {
        int size = (I+F);
        Integer op(x.value_);
...
//!
//! The fixed_point_emp value is first converted to type double and then the output

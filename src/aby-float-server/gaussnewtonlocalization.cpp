...
share* BuildGaussNewtonIteration(share *threeDPts[], share *y0[], int numPts,
    ...
    BuildInvertCircuit(jacob, 2*numPts, 6, jacobI, c, party, role, nullptr, nullptr);
    ...
    BuildInvertCircuit(jacob, 2*numPts, 6, jacobI, (BooleanCircuit*) c, party, role, toRawShares, toShareObjects);
    ...
    share *dx[6];
    BuildMatmultCircuit(jacobILinear, 6, 2*numPts,
...
bool RunGaussNewtonIteration(uint32_t *threeDPts, uint32_t *y0, int numPts,
    ...
    // prepare circuit
    share *s_done = BuildGaussNewtonIteration(s_threeDPts, s_y0, numPts, s_f, s_cx, s_cy, s_x, c, p, role);
...
void RunGaussNewtonCircuit(uint32_t *threeDPts, uint32_t *y0, int numPts,
    ...
    for (int i=0; i<GN_MAX_ITR; i++) {
        ...
        if ( RunGaussNewtonIteration(threeDPts, y0, numPts, f, cx, cy, x, c, party, role) ) break;
    }
}
...
void BuildAndRunGaussNewtonLoopLeak(share *s_threeDPts[], share *s_twoDPts[], int numPts,
...
//
// x (initial guess) : 6x1 [ rotation angles, translations ]
void BuildAndRunGaussNewtonDO(share *s_threeDPts[], share *s_twoDPts[], int numPts,
                            share *s_f, share *s_cx, share *s_cy, share *s_x[],
                            BooleanCircuit *c, ABYParty* party, e_role role) {
    // First transform input shares
    //
    ...
    for (int i=0; i<GN_MAX_ITR; i++) {
        ...
        share *s_done = BuildGaussNewtonIteration(s_threeDPts, s_y0, numPts, s_f, s_cx, s_cy, s_x, c, party, role);
        delete s_done; // throw it away
    }
}

void BuildAndRunGaussNewton(share *s_threeDPts[], share *s_twoDPts[], int numPts,
                            share *s_f, share *s_cx, share *s_cy, share *s_x[],
                            BooleanCircuit *c, ABYParty* party, e_role role) {
#if PPL_FLOW==PPL_FLOW_DO
    BuildAndRunGaussNewtonDO(s_threeDPts, s_twoDPts, numPts,
                            s_f, s_cx, s_cy, s_x,
                            c, party, role);
#elif PPL_FLOW==PPL_FLOW_LOOP_LEAK
    BuildAndRunGaussNewtonLoopLeak(s_threeDPts, s_twoDPts, numPts,
                            s_f, s_cx, s_cy, s_x,
                            c, party, role);
#endif
}

uint32_t test_gaussnewton_circuit(e_role role, const std::string& address, uint16_t port, seclvl seclvl,
    ...
    BuildAndRunGaussNewton(s_threeDPts, s_twoDPts, numPts,
                            s_f, s_cx, s_cy, s_x,

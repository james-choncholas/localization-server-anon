...
void BuildSvdPart1Circuit(share **a[], int nRows, int nCols,
        share *w[], share **v[], share **anorm, share *rv1[],
        BooleanCircuit *c) {
    ...
    share *f, *g, *h, *s, *scale; // local secrets (are reset before being used elsewhere)
    ...
    *anorm = new boolshare(zero_gate->get_wires(), c);//copy to new share
    ...
    for (i=0; i<nCols; i++) {
        ...
        if (i < nRows) {
            for (k = i; k < nRows; k++) {
                boolshare *shcopy = new boolshare(a[k][i]->get_wires(), c);//copy to new share
        ...
        if (i < nRows && i != nCols - 1) {
            for (k = l; k < nCols; k++) {
                boolshare *shcopy = new boolshare(a[i][k]->get_wires(), c);//copy to new share
                BuildFabsCircuit(shcopy, c);
                scale = c->PutFPGate(scale, shcopy, ADD, bitlen, 1, no_status);
            }
            { // if (scale)
                ...
                f = new boolshare(a[i][l]->get_wires(), c); // not used again, no mux okay
                temp = c->PutFPGate(s, SQRT);
                BuildSignCircuit(temp, f, c);
                BuildNegativeCircuit(temp, c);
                g = c->PutMUXGate(temp, g, ifscale);
                ...
                h = c->PutMUXGate(h, htoone, ifscale);// h is used to set rv1 if scale==0, set to 1 for noop
                temp = c->PutFPGate(f, g, SUB, bitlen, 1, no_status);
                a[i][l] = c->PutMUXGate(temp, a[i][l], ifscale);
                for (k = l; k < nCols; k++) {
                    rv1[k] = c->PutFPGate(a[i][k], h, DIV, bitlen, 1, no_status); // no mux because h is one
                }
                for (j = l; j < nRows; j++) {
                    s = new boolshare(zero_gate->get_wires(), c);
                    for (k = l; k < nCols; k++) {
        ...
        boolshare *shcopy1 = new boolshare(w[i]->get_wires(), c);//copy to new share
        boolshare *shcopy2 = new boolshare(rv1[i]->get_wires(), c);//copy to new share
            ...
            { //if (g)
                ...
                for (j = l; j < nCols; j++) {
                    ...
                    for (k = l; k < nCols; k++) {
                        ...
                        s = c->PutFPGate(s, temp, ADD, bitlen, 1, no_status); // s only used here so no mux okay
                    }
                    for (k = l; k < nCols; k++) {
    ...
    for (i = IMIN(nRows,nCols) - 1; i >= 0; i--) {
        ...
        { //if (g)
            ...
            for (j = l; j < nCols; j++) {
                ...
                for (k = l; k < nRows; k++) {
                    temp = c->PutFPGate(a[k][i], a[k][j], MUL, bitlen, 1, no_status);
                    s = c->PutFPGate(s, temp, ADD, bitlen, 1, no_status); // s is only used here so no mux gate okay
                }
                temp = c->PutFPGate(s, a[i][i], DIV, bitlen, 1, no_status);
                f = c->PutFPGate(temp, g, MUL, bitlen, 1, no_status); // f is only used here so no mux gate okay
                for (k = i; k < nRows; k++) {
                    temp = c->PutFPGate(f, a[k][i], MUL, bitlen, 1, no_status);
                    temp = c->PutFPGate(a[k][j], temp, ADD, bitlen, 1, no_status);
                    a[k][j] = c->PutMUXGate(temp, a[k][j], ifg); // do nothing if(g) is false
                }
            }
            for (j = i; j < nRows; j++) {
...
void RunSvdPart1Circuit(uint32_t **a, int nRows, int nCols,
        uint32_t *w, uint32_t **v, uint32_t *anorm, uint32_t *rv1,
...
share* BuildSvdCheckZeroCircuit(share *val, share *anorm, BooleanCircuit *c) {
    ...
    BuildFabsCircuit(val, c); // overwriting is okay because this is separate circuit
    share* vpa = c->PutFPGate(val, anorm, ADD, bitlen, 1, no_status);
    share* gt = c->PutFPGate(vpa, anorm, CMP); // = val+anorm > anorm (val always > 0)
...
void BuildSvdFlag1Circuit(share **cc, share **f, share **s, share **rv1i, BooleanCircuit *c) {
    *f = c->PutFPGate(*s, *rv1i, MUL, bitlen, 1, no_status);
    *rv1i = c->PutFPGate(*cc, *rv1i, MUL, bitlen, 1, no_status);
}
void RunSvdFlag1Circuit(uint32_t *cc, uint32_t *f, uint32_t *s, uint32_t *rv1i, BooleanCircuit *c, ABYParty *p) {
...
void BuildSvdFlag2Circuit(share *aSTARi[], share *aSTARnm[], int nRows,
        share **wi, share **cc, share **f, share **s, BooleanCircuit *c) {
...
void RunSvdFlag2Circuit(uint32_t *aSTARi, uint32_t *aSTARnm, int nRows,
        uint32_t *wi, uint32_t *cc, uint32_t *f, uint32_t *s, BooleanCircuit *c, ABYParty *p) {
    std::vector<Sharing*>& sharings = p->GetSharings();
    BooleanCircuit* bc = (BooleanCircuit*)sharings[S_BOOL]->GetCircuitBuildRoutine();
...
void BuildSvdZCircuit(int nCols, share **z, share **wk, share *vSTARk[], BooleanCircuit *c) {
...
void RunSvdZCircuit(int nCols, uint32_t *z, uint32_t *wk, uint32_t *vSTARk, BooleanCircuit *c, ABYParty *p) {
...
void BuildSvdPart2Circuit(share **a[], int nRows, int nCols,
        share *w[], share **v[], share *rv1[], share **z,
        int k, int l, int nm, BooleanCircuit *c) {
...
void RunSvdPart2Circuit(uint32_t **a, int nRows, int nCols,
        uint32_t *w, uint32_t **v, uint32_t *rv1, uint32_t *z,
        int k, int l, int nm, BooleanCircuit *c, ABYParty *p) {
    ...
    // prepare circuit
    BuildSvdPart2Circuit(s_a, nRows, nCols, s_w, s_v, s_rv1, &s_z,
            k, l, nm, c);
...
void RunSvdLoopLeak(uint32_t **a, int nRows, int nCols, uint32_t *w, uint32_t **v,
...
void BuildAndRunSvdLoopLeak(share ***s_a, int nRows, int nCols, share **s_w, share ***s_v,
    ...
    //// next run another dummy circuit to get the cleartext output from shared output
    ...
    toShareObjects(); // allow caller to recover shares across sub-circuit executions
}
    ...
    for (k = nCols - 1; k >= 0; k--) {
        ...
        for (its = 0; its < 30; its++) {
            ...
            for (l = k; l >= 0; l--) {
                ...
                share *rv1lcopy = new boolshare(rv1[l]->get_wires(), c); // check zero overwrites with Fabs
                ...
                if (nm >= 0) {
                    share *wnmcopy = new boolshare(w[nm]->get_wires(), c); // check zero overwrites with Fabs
            ...
            share* priv_nm = c->PutSUBGate(priv_l, int_one_gate); // integer subtraction
            { //if (flag)
                ...
                for (i = 1; i <= k; i++) {
                    ...
                    share* fcopy = new boolshare(f->get_wires(), c); // check zero overwrites with Fabs
                    ...
                    for (int j = 0; j < nRows; j++) {
                        for (int nm_finder=0; nm_finder<k; nm_finder++) {
                            share* priv_nm_finder = c->PutCONSGate((uint32_t*) &nm_finder, bitlen);
                            ...
                            tmp1 = c->PutFPGate(z, cc, MUL, bitlen, 1, no_status);
                            tmp2 = c->PutFPGate(y, s, MUL, bitlen, 1, no_status); // y is broken?
...
uint32_t test_svd_circuit(e_role role, const std::string& address, uint16_t port, seclvl seclvl,
    ...
    if (role == SERVER) {
        for(int i=0; i<nRows; i++) {
            for (int j=0; j<nCols; j++) {
                s_a[i][j] = bc->PutINGate((uint32_t*) &a_in[i][j], 32, role);
            }
        }
    } else {
        for(int i=0; i<nRows; i++) {
    ...
    BuildAndRunSvd(s_a, nRows, nCols, s_w, s_v, (BooleanCircuit*)circ, party, role);

    // shared outputs from circuit
    //  again always use boolean shares (not yao) because output is coming directly
    //  from a boolean circuit ->PutSharedInGate()
    for(int i=0; i<nRows; i++) {
        for (int j=0; j<nCols; j++) {
            share *temp = s_a[i][j];

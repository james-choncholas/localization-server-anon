...
std::pair<share*, share*> BuildLMIteration(share *threeDPts[], share *y0[], int numPts,
    ...
    // Camera params (3x3)
    share* K[9] = {f, c->PutCONSGate((uint32_t*)&zero, bitlen), cx,
                   c->PutCONSGate((uint32_t*)&zero, bitlen), f, cy,
                   c->PutCONSGate((uint32_t*)&zero, bitlen), c->PutCONSGate((uint32_t*)&zero, bitlen), c->PutCONSGate((uint32_t*)&one, bitlen)};
    ...
    for (int j=0; j<6; j++) { // for each DOF
        ...
        for (int p=0; p<2*numPts; ++p) {
            ...
            jacob[p][j] = c->PutFPGate(ytmy, jacob_epsilon_gate, DIV, bitlen, 1, no_status);
            delete ytmy;
        }
    ...
    BuildMatmult2DwTransposeCircuit(jacob, numPts*2, 6, true,
                                    ...
                                    JtJ, c); // must use 2D arrays, not 1D
    // add lambda*diag(Jt*J) in-place
    for (int p=0; p<6; p++) {
        // add fletcher column-wise to JtJ
        share* fletcher = c->PutFPGate(lambda, JtJ[p][p], MUL, bitlen, 1, no_status);

        for (int pp=0; pp<6; pp++) {
            share* temp = JtJ[pp][p];
            JtJ[pp][p] = c->PutFPGate(JtJ[pp][p], fletcher, ADD, bitlen, 1, no_status);
            delete temp;
        }
        delete fletcher;
    }
    ...
    std::function<void()> toRawShares = [dy, raw_dy, jacob, raw_jacob, x, raw_x, lambda, raw_lambda, prevErrNorm, raw_prevErrNorm, numPts](){
        for (int i=0; i<2*numPts; ++i) {
    ...
    std::function<void()> toShareObjects = [dy, raw_dy, jacob, raw_jacob, x, raw_x, ref_lambda, raw_lambda, ref_prevErrNorm, raw_prevErrNorm, numPts, bc](){
        for (int i=0; i<2*numPts; ++i) {
    ...
    BuildInvertCircuit(JtJ, 6, 6, JtJ_i, (BooleanCircuit*) c, party, role, toRawShares, toShareObjects);
    ...
    share *dx[6];
    BuildMatmultCircuit(JtJ_i_Jt_linear, 6, 2*numPts,
                        dy, 2*numPts, 1, //column vector
                        dx, c);
    ...
    share* er_flag = c->PutFPGate(min_er_gate, absnorm, CMP, bitlen, 1, no_status);
    if (_verbosity & DBG_ER) {
        c->PutPrintValueGate(absnorm, "absnorm");
    }

    share* biggerLambda = c->PutFPGate(lambda, ten_gate, MUL, bitlen, 1, no_status);
    share* smallerLambda = c->PutFPGate(lambda, ten_gate, DIV, bitlen, 1, no_status);

    share* errWentDown = c->PutFPGate(prevErrNorm, absnorm, CMP, bitlen, 1, no_status);
    share* oldlam = lambda;
    ...
    share* lambdaTooBig = c->PutFPGate(lambda, lambda_max_gate, CMP, bitlen, 1, no_status);
    oldlam = lambda;
    lambda = c->PutMUXGate(lambda_max_gate, lambda, lambdaTooBig);
    delete oldlam;
    delete lambdaTooBig;
    share* lambdaTooSmall = c->PutFPGate(lambda_min_gate, lambda, CMP, bitlen, 1, no_status);
    oldlam = lambda;
...
bool RunLMIteration(uint32_t *threeDPts, uint32_t *y0, int numPts,
                            uint32_t f, uint32_t cx, uint32_t cy, uint32_t *x,
                            uint32_t *prevErrNorm, uint32_t *lambda,
    ...
    // build circuit
    auto s_res = BuildLMIteration(s_threeDPts, s_y0, numPts,
            s_f, s_cx, s_cy, s_x,
            s_prevErrNorm, s_lambda,
            c, p, role);
    ...
    share* s_done = c->PutFPGate(s_minErGate, s_absnorm, CMP, bitlen, 1, no_status);
    delete s_minErGate;
...
void RunLMCircuit(uint32_t *threeDPts, uint32_t *y0, int numPts,
                            uint32_t f, uint32_t cx, uint32_t cy, uint32_t *x,
                            uint32_t *prevErrNorm, uint32_t *lambda,
                            BooleanCircuit *c, ABYParty* party, e_role role) {
    //LM Iteration
    for (int i=0; i<LM_MAX_ITR; i++) {
        ...
        bool done = RunLMIteration(threeDPts, y0, numPts,
                f, cx, cy, x,
                prevErrNorm, lambda,
                c, party, role);
...
void BuildAndRunLMLoopLeak(share *s_threeDPts[], share *s_twoDPts[], int numPts,
    ...
    CLOCK(LM);
    TIC(LM);
    RunLMCircuit(threeDPts, y0, numPts,
            f, cx, cy, x,
            &prevErrNorm, &lambda,
            c, party, role);
    TOC(LM);
...
void BuildAndRunLMDO(share *s_threeDPts[], share *s_twoDPts[], int numPts,
    ...
    for (int i=0; i<LM_MAX_ITR; i++) {
        ...
        auto res = BuildLMIteration(s_threeDPts, s_y0, numPts,
                s_f, s_cx, s_cy, s_x,
                prev_err_norm_gate, lambda_gate,
                c, party, role);
...
uint32_t test_lm_circuit(e_role role, const std::string& address, uint16_t port, seclvl seclvl,
    ...
    if (role == SERVER) {
        ...
        for(int p=0; p<numPts; p++) {
            s_threeDPts[numPts+p] = bc->PutINGate((uint32_t*) &threeDPts[p].y, bitlen, role);
        }
        for(int p=0; p<numPts; p++) {
            s_threeDPts[(2*numPts)+p] = bc->PutINGate((uint32_t*) &threeDPts[p].z, bitlen, role);
        }
        //for(int p=0; p<numPts; p++) {
        //    s_threeDPts[3*numPts+p] = bc->PutINGate((uint32_t*) &one, bitlen, role);
        //}
        for(int p=0; p<numPts; p++) {
            s_twoDPts[p] = bc->PutINGate((uint32_t*) &twoDPts[p].x, bitlen, role);
        }
        for(int p=0; p<numPts; p++) {
            s_twoDPts[numPts+p] = bc->PutINGate((uint32_t*) &twoDPts[p].y, bitlen, role);
        }
        //for(int p=0; p<numPts; p++) {
        //    s_twoDPts[2*numPts+p] = bc->PutINGate((uint32_t*) &one, bitlen, role);
        //}
    ...
    BuildAndRunLM(s_threeDPts, s_twoDPts, numPts,
                            s_f, s_cx, s_cy, s_x,
